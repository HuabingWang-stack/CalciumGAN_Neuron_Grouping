...
def predict(
    ...
    for i, model_noise in enumerate(noise_levels_model):
        ...
        models = [load_model(model_path) for model_path in model_dict[model_noise]]
        ...
        XX_sel = np.reshape(
            XX_sel, newshape=(XX_sel.shape[0] * XX_sel.shape[1], XX_sel.shape[2], 1)
        )

        pred_shape = (len(neuron_idx), XX.shape[1])
        for j, model in enumerate(models):
            prediction_flat = model.predict(XX_sel, batch_size, verbose=0)
            prediction = np.reshape(prediction_flat, newshape=pred_shape)
            # average predictions
