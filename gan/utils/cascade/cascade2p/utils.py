...
Helper functions for CASCADE:
  ...
  preprocess_traces():
    converts calcium dataset to a format that can be used by the deep network
...
def preprocess_traces(neurons_x_time, before_frac, window_size):
    """
    Transform dF/F dataset into a format that can be used by the deep network.
...
def calibrated_ground_truth_artificial_noise(
    ...
    # Omit neurons from the training dataset, if indicated in the omission_list
    ...
    for file_index, neuron_file in enumerate(fileList):
        ...
        for i, trial in enumerate(dataset_neuron_all):
            ...
            # Find the relevant elements in the dataset structure
            ...
            events = events[
                ~np.isnan(events)
            ]  # exclude NaN entries for the Theis et al. dataset sets
            ...
            base_noise = (
                np.nanmedian(np.abs(np.diff(traces_mean))) * 100 / np.sqrt(frame_rate)
            )
            ...
            if nb_subROIs >= 1:
                ...
                fluo_times_bin_edges = (
                    np.append(
                        ...
                        fluo_times_bin_centers[-1] + 1 / frame_rate / 2,
                    )
                    - 1 / frame_rate / 2
                )
                ...
                for iii in range(int(nb_subROIs)):
                    ...
                    # The nested if-clause covers edge cases in some ground truth dataset sets where
                    ...
                    if np.any(sub_traces):
                        ...
                        if sub_traces.shape[0] - len(sub_traces_single) >= 0:
                            ...
                            sub_traces = np.append(
                                ...
                                np.zeros(
                                    (
                                        len(sub_traces_single) - sub_traces.shape[0],
                                        sub_traces.shape[1],
                                    )
                                )
                                + np.nan,
                            ...
                            sub_traces_events = np.append(
                                ...
                                np.zeros(
                                    (
                                        len(events_binned) - sub_traces_events.shape[0],
        ...
        # Optional output: ground truth spike times; not needed to generate a training dataset set
        try:
...
def preprocess_groundtruth_artificial_noise_balanced(
    ...
    # Go through all ground truth dataset sets and extract re-sampled ground truth
    ...
    # Reduce the number of dataset points for relatively large dataset sets to avoid bias
        ...
        if sub_traces is not None:
            ...
            for trace_index in range(sub_traces.shape[1]):
                ...
                for time_points in datapoints_used_rand:
                    ...
                    Y[
                        counter,
                    ] = single_spikes[time_points + before]
                    X[
                        counter,
                        :,
...
def plot_dFF_traces(
    ...
    """
    Plots a subset (maximum of 50 seconds) of calcium imaging dataset of a given matrix ('traces'),
    using a selection of neurons ('neuron_indices') and a 'frame_rate'.
    If spike prediction dataset are available (matrix 'spiking'), they are plotted as well.
...
def plot_noise_matched_ground_truth(
    ...
    """
    Plots a subset (junks of 50 seconds) of calcium imaging dataset together with electrophysiological
